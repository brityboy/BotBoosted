...
'''
    Entity, <-- done, this is weird stuff like ampersands
    Timelike, <-- remove time like numbers
    NumNum, <-- removes numbers
    NumberWithCommas, <-- removes num with commas
    Punct, <-- this is basic punctuation
    ArbitraryAbbrev,<-- will take care of this with the acronym dictionary
    Separators,
    Decorations,
    EmbeddedApostrophe,
'''
...
# inserted Punct_RE below
Punct_RE = re.compile("(%s)" % Punct, re.U|re.I)
# inserted Entity_RE below
Entity_RE = re.compile("(%s)" % Entity, re.U|re.I)
...
# inserted Time_RE below
Time_RE = re.compile("(%s)" % Timelike, re.U|re.I)
# inserted NumNum_RE below
NumNum_RE = re.compile("(%s)" % NumNum, re.U|re.I)
# inserted NumberWithCommas below
NumberWithCommas_RE = re.compile("(%s)" % NumberWithCommas, re.U|re.I)
# inserted Separtors_RE below
Separators_RE = re.compile("(%s)" % Separators, re.U|re.I)

# original ProtectThese is below and commented out
# ProtectThese = [
#     emoticons.Emoticon,
#     Url,
#     Entity,
...
#     EmbeddedApostrophe,
# ]
# this is the ProtectThese that I will use, Url has been removed
ProtectThese = [
    emoticons.Emoticon,
    Entity,
    Timelike,
